...
4. **各家的自帶 system prompt 還沒關掉。** 實測「只回覆兩個字：可以」這句十來個字的提示：
   ...
   **在補齊之前不要宣稱三家行為一致。**
5. **終態是二值，但應該是三值。** `回應.執行成功` 是布林，`timeout` 被歸在「失敗」。
   可是殺掉子程序時**工作可能已經做了一半**——那是**結果未知**，不是**確定失敗**。

   | 終態 | 意思 | 可以重試嗎 |
   |---|---|---|
   | 成功 | 做完了 | 不必 |
   | 確定失敗 | 沒做，而且確定沒做 | 可以 |
   | **結果未知** | **不知道做了沒** | **不可以**（at-most-once：寧漏不重） |

   把結果未知當成確定失敗，重試就會**把可能已經做過的事再做一次**。
   目前只有一個呼叫端（`nova 問`，一次性、人在看），還沒有重試迴圈，所以還沒出事。
   **迴圈層動工時這是第一批要改的東西**——不然自動續跑會是個定時炸彈。
