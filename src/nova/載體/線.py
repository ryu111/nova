...
def _護欄原因(工作樹: Path, 成果紀錄: 成果 | None) -> str | None:
    """那個 4 是哪一種護欄。**成果帳優先，狀態.json 只是舊帳的退路。**

    狀態檔的 `寫下現況` 是整檔覆寫，排程每 15 分鐘醒一次就把上一次的理由蓋掉，
    所以它答不了「昨晚那個 4 是怎麼來的」。成果帳是一執行一檔、不覆寫，
    答案住在那裡才活得夠久。

    狀態.json 這條留著是為了**這一欄之前就跑完的那些執行**：它們的成果檔
    沒有 `guardrail_reason`，但狀態檔要是剛好還停在同一次執行上，
    那句給人看的理由仍然比「查不到」有用。識別碼對不上就不准拿——
    那是下一次醒來的理由，安到上一次頭上等於記錯帳。
    """
    if 成果紀錄 is None or 成果紀錄.退出碼 != _護欄退出碼:
        return None
    if 成果紀錄.護欄原因 is not None:
        return 成果紀錄.護欄原因
    現況 = 讀現況(狀態檔(工作樹))
