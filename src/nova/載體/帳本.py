...
| ⚠️ 出聲 | 兩個程序**開同一個檔名**（撞號） | `"x"` 開檔，撞到就當場炸（不是靜默合併） |
| ❌ 不防 | 兩個程序**共寫同一個 handle** | 一次執行一個檔、一個 handle（見下方「從簡」） |
...
def 新執行識別碼() -> str:
    ...
    **格式擋得住亂設，擋不住歸屬。** 一個格式完全合法、但屬於上一次執行的
    值，這裡照樣會回給你——所以真的撞到的時候由 `開帳本` 用 `"x"` 當場炸，
    不是在這裡擋。兩層分工：這裡管「像不像識別碼」，那裡管「這個號有人用了沒」。
    """
...
def 開帳本(目錄: Path | None = None, *, 執行識別碼: str | None = None) -> Iterator[帳本]:
    ...
    **用 `"x"` 開（`O_CREAT|O_EXCL`），撞號一律出聲。** 本來是 `"a"`，
    名義上為了「續接同一次執行」——但三個呼叫端沒有任何一個在續接，
    一次執行從頭到尾只開一次檔。所以 `"a"` 真正的效果是把**兩次不同執行**
    的事件交錯合併進同一本帳，而合併之後沒有人分得出哪一行是哪一次跑的。

    真的出現分多段寫的呼叫端那天再加一個明確的 `續接` 參數，不要現在先造。
    """
    在哪 = 目錄 or 預設帳本目錄()
    識別 = 執行識別碼 or 新執行識別碼()
    ...
    落點 = 在哪 / f"{識別}.jsonl"
    try:
        檔 = 落點.open("x", encoding="utf-8")
    except FileExistsError as 錯:
        訊息 = (
            f"執行識別碼撞號：{落點} 已經在了。"
            f"多半是 {指定識別碼的環境變數} 殘留在環境裡——清掉它再跑一次。"
        )
        raise FileExistsError(訊息) from 錯
    with 檔:
