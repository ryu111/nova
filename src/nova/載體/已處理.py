...
class 跨專案的一筆:
    """一筆成果，加上「它是哪個專案的」與它的事件帳本在哪。

    掃全部專案的時候，光有 `成果` 說不出它躺在哪個目錄底下——
    而事件帳本的檔名是 `執行識別碼`、目錄卻是**那個專案自己的**，
    所以這兩格要跟著成果一起走，不能事後從 cwd 推。
    """
...
def 列出所有專案成果(*, 上限: int | None = None) -> list[跨專案的一筆]:
    """掃過所有專案的成果帳，`迄` 最近的排前面。

    **一個專案只看得到自己那幾筆。** 每棵派工樹是自己一個 `專案識別`，
    同一種收場散在幾十個目錄裡；只掃 cwd 那個等於問「今晚重覆了嗎」
    卻只翻自己那一頁。

    **讀不動就往上丟 `OSError`，不吞成空清單**：`glob` 碰到列不動的目錄會
    安靜回空的，那會讓「看不到」跟「沒東西可看」長得一模一樣。
    根目錄不存在是另一回事（還沒有任何專案），那才是真的空。
    """
    根 = 狀態根目錄() / "專案"
    專案們 = sorted(根.iterdir()) if 根.is_dir() else []
    收集: list[跨專案的一筆] = []
    for 專案目錄 in 專案們:
        收集.extend(_一個專案的成果們(專案目錄))
    收集.sort(key=lambda 它: 它.一筆.迄, reverse=True)
    return 收集 if 上限 is None else 收集[:上限]


def _一個專案的成果們(專案目錄: Path) -> list[跨專案的一筆]:
    """一個專案目錄底下的成果帳。沒有 `已處理/` 就是空的——那個專案還沒收過場。"""
    已處理 = 專案目錄 / "已處理"
    if not 已處理.is_dir():
        return []
    _先確認列得動(已處理)
    return [
        跨專案的一筆(
            專案識別=專案目錄.name,
            一筆=一筆,
            事件帳本=專案目錄 / "帳本" / f"{一筆.執行識別碼}.jsonl",
        )
        for 一筆 in 列出成果(已處理, 上限=None)
    ]


def _先確認列得動(目錄: Path) -> None:
    """碰一下這個目錄，列不動就當場丟 `OSError`。

    `列出成果` 走 `glob`，而 `glob` 對列不動的目錄安靜回空的。
    少算的方向永遠是「看起來還沒到門檻」，所以寧可炸。
    """
    next(iter(目錄.iterdir()), None)
