...
#:
#: `pytest-parallel` 的排除清單從這裡推出來（見下面的 `負控檔排除參數`），
#: 所以新增負控檔只要加在這一行，兩條規則會同時看到它，
#: 不會出現「負控跑了、平行忘了排除」而重複跑一次的分岔。
負控檔們: tuple[str, ...] = ("tests/負控/test_登記的變異會被殺.py",)

#: `pytest-parallel` 排掉負控檔用的 `--ignore` 參數，由 `負控檔們` 推出來。
#:
#: 負控檔歸 `registered-mutation` 跑，平行那條再收一次是同一件事做兩遍：
#: 實測拿掉這次重複，`pytest-parallel` 61.97 → 17.62 秒。
