...
def _外部指令(根目錄: Path, *指令: str) -> Callable[[], tuple[bool, str]]:
    ...
    # ruff 兩條都帶 `--no-cache`，理由不是潔癖：**ruff 的快取鍵是（路徑、大小、
    # mtime 奈秒），不含內容**。三者的前兩項一樣、內容不一樣就會拿到上一次的結論，
    # 於是閘全綠、推上去 CI 當場紅——假的安全感比紅燈更糟。
    # 實測本專案 95 個檔：有快取 26 毫秒、沒快取 27 毫秒。買到 1 毫秒，
    # 賣掉的是「閘綠等於 CI 綠」。由 tests/整合/test_閘不准吃ruff快取.py 背書。
