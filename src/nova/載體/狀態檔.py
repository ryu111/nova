...
#:
#: **`等重置` 不能跟 `沒事做` 壓成同一個**：兩者都是「這一輪什麼都沒跑」，
#: 但一個是「收件匣真的空著」（該去生票），一個是「票在，時鐘還沒到」
#: （什麼都不要做）。壓成同一個就會每次醒來又生一張票、下次再撞同一個額度。
做完了, 沒事做, 被預算擋, 在忙, 出不了生, 壞了, 等重置, 巡過了 = (
    "done",
    "idle",
    "budget",
    "busy",
    ...
    "error",
    "waiting",
    # 巡自己那一筆：它不跑工作流，只是把到期的票叫起來，
    # 寫成 `done` 會讓狀態列說「做完一件」——這一輪它一件都沒做。
    "patrol",
)
...
class 現況:
    ...
    **佇列深度與卡住幾件不在這裡**：那是「現在」的事實，存進檔案就會過期
    ...
    上次理由: str = ""
    上次執行識別碼: str = ""
    #: 這條線最早什麼時候可以再接（UTC ISO）。撞到上游額度那一輪才有值。
    #: **人與未來的掃描器共用同一個時刻**：寫在這裡，`nova 線` 一眼看得到，
    #: 不必去翻票的標記，也不必自己把 `resets 4:40pm` 換算成 UTC。
    不早於: str = ""
