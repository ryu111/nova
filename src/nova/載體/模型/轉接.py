...
#: codex 只用這兩顆（使用者裁定）。luna 是常用的，sol 是高階推理。
#: **字串本身住在 `型號家族`**——判斷「這顆屬不屬於 codex」要看得見別家的型號，
#: 散在三個檔案就查不起來。這裡轉出來給派工表與測試沿用原本的引用路徑。
codex常用模型 = 型號家族.codex常用模型
codex高階模型 = 型號家族.codex高階模型
#: agy 的推理強度包在型號裡（`agy models` 實測），不是另一個旗標。同上，字串住 `型號家族`。
agy預設模型 = 型號家族.agy預設模型
#: 高階目前只有一顆；只給派工表斷言使用。
...
def _agy型號(選項: 呼叫選項) -> str:
    ...
    **但只有 gemini 那一族是這個規則。** agy 也代跑 claude 與 gpt 的型號
    ...
    走不到等於整池浪費（`型號家族.agy認得的型號` 就是照這件事列的）。
    """
    型 = 選項.模型 or agy預設模型
...
class 命令列模型:
    ...
    def 詢問(
        ...
        *,
        ...
        `選項.模型` 通過家族檢查後原樣傳下去不翻譯；各家的模型命名空間不交集，
        硬翻譯只會翻錯。
...
def 建命令列(家: 家族, *, 執行檔: Path | None = None) -> 命令列模型:
    ...
    **不要用 cast 從 `建立` 的回傳值硬轉**：那等於把型別檢查關掉。
    ...
    def 組並先擋(提示: str, 選項: 呼叫選項) -> list[str]:
        ...
        深度擋在組參數這一層、而不是命令列那一層：擋在上面的話，繞過命令列
        直接建腦的呼叫端會漏掉這道檢查，而漏掉的症狀是「安靜地沒有生效」，
        那是最難發現的一種。型號的家族檢查在 `命令列模型._問一次`——
        那一層連自己組 `命令列模型` 的呼叫端都蓋得到。
        """
        _檢查深度(家, 選項.思考深度)
        return 組(提示, 選項)
