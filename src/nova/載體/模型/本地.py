...
from nova.載體.模型.型號家族 import 檢查型號
...
#:
#: 8 → 16 → **12**（2026-09-01 同一天調兩次，第二次是因為**綁住的東西換了**）。
#:
#: 8 太少：帳本全量顯示本地腦撞上限 19 次、燒掉 1,931,437 token 而零產出。
#:
#: 16 是往上喊過頭：同一天 oMLX 那邊實測 64K 冷請求會踩到記憶體 hard line
#: （49.6 GB vs 49.2 GB）並 abort，所以這顆模型的 `max_context_window`
#: 被收到 **32768**。回合上限一旦超過 context 撐得住的回合數，
#: **綁住的就不再是回合而是 context**——而那兩種收場差很多：
#: 撞回合上限是護欄（退出碼 4，設計上的停止），撞 context 是錯誤或截斷。
#:
#: 12 怎麼來的（**外推，不是實測，只有 6 個資料點**）：sol 量到的 prompt token
#: 是**跨回合累加**，`low/false` 三回合共 13,910、`medium/false` 四回合共 26,725。
#: 當成線性成長 p, 2p, 3p… 反推每回合增量約 2,300–2,700，
#: 於是第 12 回合前後的單次 prompt 才會逼近 32K。
#:
#: 天花板：這個數字綁在 `max_context_window` 上，而那個值寫在
#: `~/.omlx/model_settings.json`——**nova 讀不到它，所以兩邊會走散**。
#: 正解是從 `/v1/models` 的 `max_model_len` 推導回合上限，不是再猜一個常數；
#: 收件匣有票（`回合上限該從context預算推導`）。
最多工具回合 = 12
...
#: 建構列指名這一顆。**型號住在它自己家族旁邊**，派工表只引用，不自己拼字串。
#: （codex／agy 那幾顆改住 `模型/型號家族.py`，因為歸屬檢查要拿它們互相比對；
#: 本機這顆不必——本機接受任意型號，那份對照表列不完它。）
#:
#: 為什麼要指名：`_第一個型號()` 拿的是 `/v1/models` 清單的**第一個**，
#: 而那個順序是端點決定的——多下載一顆模型就會靜靜換腦，帳本還看不出來
#: （帳本只記家族名不記型號）。**「清單第一個」不是一個決定，是一個意外。**
建構型號 = "Qwen3.8-27B-8bit"
...
class 本地腦:
    ...
    def 詢問(self, 提示: str, *, 選項: 呼叫選項 = 預設選項) -> 回應:
        ...
        型號的家族歸屬也在這裡擋——擋在 `轉接` 那層的話走 HTTP 的本地腦整個漏掉。
        """
        檢查型號(家族名, 選項.模型)
        做不到 = _做不到的地方(選項)
        if 做不到 is not None:
