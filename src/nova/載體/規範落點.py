...
規範登記表: tuple[規範登記, ...] = (
    ...
    規範登記(
        "宣告:18",
        規範狀態.有測試背書,
        "判準是對話不是家族名；`固定提示角色` 沒有續接欄位，結構上不可能自寫自評。",
        標籤="不得讓同一個對話自寫自評",
        測試nodeid="tests/單元/test_換腦判準.py::test_角色結構上不可能續接到同一個對話",
    ),
    規範登記(
        "宣告:19",
        規範狀態.還是懇求,
    ...
    規範登記(
        "宣告:20",
        規範狀態.有測試背書,
        "測試員那一階會被指名補負控登記，但補了沒有還沒有閘擋。",
        測試nodeid=(
            "tests/單元/test_測試員要補負控登記.py"
            "::Test新增保證的票要在測試員那一階補負控登記"
            "::test_新增保證的票送出去的提示會指名登記檔補一列"
        ),
        標籤="新增保證要做一次固定負控",
    ),
    規範登記(
