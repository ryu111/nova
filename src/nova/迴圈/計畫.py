...
分派單結構 = 結構識別碼("分派單")
分派單版本 = 1
#: 餵進計畫員那一包的結構。**跟產出的 `分派單結構` 分開兩個名字**：
#: 兩邊共用一個識別碼的話，邊上的結構欄就再也分不出這包是題目還是答案。
規劃輸入結構 = 結構識別碼("規劃輸入")
規劃輸入版本 = 1


def 來源自上下文(上下文: 節點上下文) -> 證據來源:
    """把節點上下文的追溯欄位照抄成證據來源。

    兩邊欄位同名同義，抄漏一欄就是斷掉一條追溯線，所以只留這一份抄法。
    """
    return 證據來源(
        執行=上下文.執行,
        工作流=上下文.工作流,
...
class 計畫員節點:
    ...
    包這一層的理由只有一個：`迴圈/節點.py` 的 `單獨執行` 才管得到這一次呼叫的
    停止政策（步數、逾時、`用量` 對上 `最多token` 的預算護欄）。直接呼叫
    `執行計畫員` 的話，那顆模型花多少都沒有人擋。
    """
    ...
    def 執行(
        ...
        *,
        ...
        """跑一次計畫員，產出一張**還沒驗過**的候選分派單。"""
        return 執行計畫員(輸入, 上下文=上下文, 依賴=依賴)
