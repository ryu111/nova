...
def _檢查寫入範圍(項目們: tuple[分派項目, ...]) -> tuple[分派單問題, ...]:
    """擋掉會並行、卻寫同一塊地的項目——沒有資料依賴不等於獨立。

    沒宣告寫入範圍的項目不參與比對（空 tuple 的雙層迴圈本來就不進去），
    理由寫在 `契約/分派.py` 的 `寫入範圍` 上。
    """
    問題: list[分派單問題] = []
    for 甲位, 甲 in enumerate(項目們):
        for 乙 in 項目們[甲位 + 1 :]:
            if _會循序跑嗎(甲, 乙):
                continue
            問題.extend(_相交問題們(甲, 乙))
    return tuple(問題)


def _會循序跑嗎(甲: 分派項目, 乙: 分派項目) -> bool:
    """兩格是不是同一個分支上的先後兩步。

    循序跑的兩格寫同一塊地不是假獨立，後寫的蓋掉先寫的正是它該有的樣子。
    都沒有分支代表會並行，不算循序。
    """
    return 甲.分支 is not None and 甲.分支 == 乙.分支


def _相交問題們(甲: 分派項目, 乙: 分派項目) -> tuple[分派單問題, ...]:
    """兩格之間每一對撞在一起的寫入範圍各報一個問題。

    比路徑節點而不是字元前綴的那一格直接複用扇出的 `_撞在一起嗎`：
    同一個 repo 裡兩套寫法，遲早有一套會被改壞而沒人發現。
    """
    return tuple(
        分派單問題(
            分派問題代碼.寫入範圍相交,
            甲.識別碼,
            f"項目 {甲.識別碼} 與 {乙.識別碼} 的寫入範圍撞在一起"
            f"（{甲格} 與 {乙格}）——沒有資料依賴不等於獨立",
        )
        for 甲格 in 甲.寫入範圍
        for 乙格 in 乙.寫入範圍
        if _撞在一起嗎(甲格, 乙格)
