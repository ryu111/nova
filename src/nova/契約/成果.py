...
_欄位對照: tuple[tuple[str, str, Any], ...] = (
    ...
    ("最後階段", "last_stage", None),
    ("跳開理由", "skip_reason", None),
    ("驗收", _驗收鍵, None),
)
...
class 成果:
    ...
    **任務也要先過遮罩。** 以前的理由是「它已經在 `ps` 的 args 上，
    ...
    護欄原因: str | None = None
    #: 這一輪**最後到達**的階段（`契約.工作流.階段代碼` 的值）。
    #: 「有沒有進展」的機械證據就是這一格：下一輪要不要排，比的是它有沒有
    #: 比上一輪那筆往後挪。沒有這一欄的話，「連撞三次同一面牆」與
    #: 「一輪比一輪走得遠」在帳上長得一模一樣。
    #: 型別是 `str` 不是 `階段代碼`：帳從 JSON 讀回來就只是字串。
    最後階段: str | None = None
    #: 這一輪撞了護欄卻**不再排下一輪**的理由（原因換了、或沒進展）。
    #: `None` 是正常狀態——沒跳開就沒有這一格。
    #: 跳開是「不再自動重試、留給人」的狀態轉換，而狀態轉換要記在
    #: 活得久的那本帳上；`狀態.json` 15 分鐘就被蓋掉，記在那裡等於沒記。
    跳開理由: str | None = None
