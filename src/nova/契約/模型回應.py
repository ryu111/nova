...
_終局表: dict[失敗代碼, 終局] = {
    ...
    # 請求被拒，**或跑到一半被拒**：2026-09-02 實測 claude 是跑了 71～218 秒、
    # 吐了 4k～16k output token 之後才撞到 session limit（工具迴圈跑在半路）。
    # 所以這一列的「確定失敗」是**確定沒做完**，不是「沒出門、沒有副作用」。
    # 接力可以換下一家；迴圈那一層另外以額度短路（見 `契約.工作流.結束代碼.額度`）。
    # at-most-once 的邊界由「接續同一階」守，不由這一列守。
    失敗代碼.額度耗盡: 終局.確定失敗,
