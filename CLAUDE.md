...
「它說的話唯一的出路是 journal」現在做了一半：`載體/帳本.py` 會把每次執行寫成
append-only 的 jsonl，`載體/模型/記帳.py` 記模型呼叫、`載體/階段記帳.py` 記階段。
**只有寫端沒有讀取端，所以只能說補了 Evidence，不能說補了 State。**
而且**不記模型全文**（repo public，遮罩機制還不存在），只記長度與雜湊——
所以它擋不住「模型說了不該說的話」，只回答得了「誰被叫了、花多少、怎麼收場」。
