...
平行只發生在 pytest 內部（`-n min(4, 3/4 核心) --dist worksteal`），且 `serial` 標記的測試單獨序列跑。**worker 不是越多越快**——實測 16 核開 12 個比開 4 個慢 1.1 秒，數字與重量時機見 [`docs/設計/05-測試怎麼跑最快.md`](docs/設計/05-測試怎麼跑最快.md)。
**不吃滿核心**：實測 16 核這台，worker 越多越慢（4 個 3.44 秒、12 個 4.02 秒、16 個 4.25 秒）。
**變慢是量到的，原因還沒量到**——候選解釋見 05，別把它當結論。所以 4 是這台調出來的
數字不是通則，換機器要重量。3/4 是留餘裕的通則，調整點是 `規則表.平行成數`。
...
其中「預算鎖」在**工作流**這一層已經有了（`契約/工作流.py` 的 `停止條件`：
步數上限＋token 上限，累計式、下一次呼叫之前擋下，預設 500k）。
`nova 問` 這條單次呼叫的路徑**還沒有**——它一次只發一個請求，沒有累計的對象。

「它說的話唯一的出路是 journal」現在有了：`載體/帳本.py` 寫、`載體/帳本讀取.py` 讀
（`uv run nova 帳本`），`載體/模型/記帳.py` 記模型呼叫、`載體/階段記帳.py` 記階段。
**Evidence 完整了，但還不是 State**——沒有任何程式碼會因為帳本裡的東西改變行為，
跨執行熔斷與 checkpoint 都還沒有。而且**不記模型全文**（repo public，遮罩還不存在），
只記長度與雜湊，所以它回答得了「誰被叫了、花多少、怎麼收場」，
回答不了「它說了什麼」。
...
退出碼分四種：**0 成功、1 確定失敗、3 結果未知、4 護欄生效**（工作流才有）。**4 代表「停止規則生效了，不是壞了」**（預算、步數、卡住、重構改壞行為）——
外圈看到 4 **不准去「修」**：護欄最省事的修法是把上限調高，那是自己拆執法點。
要放寬是人的決定。3 代表「不知道工作做了沒」
...
TDD 七階段：`測試(模型) → 驗證紅(機械) → 實作(模型) → 驗證綠(機械) → 重構(模型) → 驗證重構(機械) → 審查(模型，換一顆腦)`。
**驗證是機械判準不是角色**——硬規則 4 禁止自寫自評；而且它出現**三次**。
接線走 `建TDD執行器(角色表, 跑判準)`：加階段是多一列資料，不是改簽章。
