...
uv run nova 問 --工作 routine|reasoning "提示"          # 照派工表挑腦
...
uv run nova 問 --用 codex --保留對話 "提示"             # 留下 sid，下一輪 --續接 <sid>
...
它就在模型手上。判準與五格盤點見 [`docs/設計/06`](docs/設計/06-IHarness要補什麼.md)。

`nova 問` 不是走錯路：它是「執行核」第 ② 步的雛形，缺的是包住它的 ①③④——
**出生前就擋**（秘密、預算鎖、熔斷）、**它說的話唯一的出路是 journal**、**終態分類**。
預算鎖在**工作流**那一層已經有了（`契約/工作流.py` 的 `停止條件`）；
...
**帳本影響行為的路徑已存在（熔斷是第一條）**：`src/nova/載體/熔斷.py` 的 `該跳過嗎` 已由
`src/nova/載體/命令列.py` 的 `_這個專案誰熔斷了` 與 `_濾掉熔斷的` 接進 `nova 問` 接力鏈；**預設被旗標鎖住**，
明講 `nova 問 --熔斷` 才啟用；只讀最近 10 次執行，讀不到帳本 fail-open 不熔斷。帳本仍繼續記、繼續讀，
**checkpoint 仍然沒有**；而且**不記模型全文**（repo public，遮罩還不存在），只記長度與雜湊，
...
```
                    住在專案底下          住在專案外面
會被餵回模型      ❌ 執行者可以種話      ✅ 進度檔走這條
（進度檔、前情）      給未來的自己看          （檢查進度檔位置 擋著）

只給人看          ✅ 記錄跟著專案走       ⚠️ 只有這樣的話
（帳本、log）                                各專案的帳全混在一起
```
...
2. **只給人看的東西，要按專案分。** 屬於哪個專案是**索引**問題，
   不是**存放位置**問題：存在專案外面、用專案當鍵，兩件事同時成立。

nova 用到新專案時，那個專案的進步與紀錄都歸那個專案。
...
**各家自帶的載體一律關到最小**（工具、session、家目錄設定）——依賴它們會讓
nova 的行為變成「這次剛好用了哪一家的行為」。哪幾條旗標做這件事由
`tests/整合/test_模型轉接.py::Test把各家載體關到最小` 背書。
**旗標、型號、各家的坑全在 `docs/設計/02`，不要抄一份到這裡。**

**真 CLI 測試不能省**（`pytest -m 真cli`）：**墊片證明的是轉遞形狀，不是可達性。**
