...
_子命令的最小參數 = {
    ...
    "跑": ["跑", "任務"],
    "派工": ["派工", "票.md"],
    # `巡` 的 `--專案` 是必填：不給就 parse 不過，窮舉測試連 `執行` 都問不到。
    "巡": ["巡", "--專案", "專案"],
    "排程": ["排程"],
    "狀態": ["狀態"],
    "秘密": ["秘密"],
...
class Test子命令只准有一份登記來源:
    ...
    @pytest.mark.parametrize("缺的", sorted(處理們))
    def test_少任何一格都在建剖析器時炸(self, 缺的: str) -> None:
        """**每一格都一樣。**

        對某個名字寫 `if 名 in 處理們:` 就是把它排除在
        這條規則之外：處理表漏了它，剖析器安靜地少宣告一個子命令，
        使用者打下去拿到的是 `invalid choice`，看起來像「這個功能不存在」，
        而不是「登記漏了一格」。**放行一格等於這條規則對所有格子都只是建議。**
        """
        少一格 = {名: 函 for 名, 函 in 處理們.items() if 名 != 缺的}
        with pytest.raises(KeyError, match=缺的):
            建剖析器(少一格)

    def test_處理們少了儀表板建剖析器要炸(self) -> None:
        """**同一條規則，寫成獨立的一支。**

        上面那支是參數化的，pytest 會把中文的參數 id 轉義成一串跳脫碼，
        判準讀不出是哪一格紅，只好收「結果未知」。這一格是最新加的、
        也是唯一被 `if "儀表板" in 處理們:` 特判放行的一格，
        所以它的紅要**看得懂**：函式名不會被轉義。

        特判在的時候，處理表漏了儀表板，`建剖析器` 什麼都不說地少宣告
        一個子命令——使用者打 `nova 儀表板` 拿到 `invalid choice`，
        像是「這個功能不存在」，而不是「登記漏了一格」。
        """
        少一格 = {名: 函 for 名, 函 in 處理們.items() if 名 != "儀表板"}
        with pytest.raises(KeyError, match="儀表板"):
            建剖析器(少一格)
