...
class Test做不到就明講:
    ...
    本地模型沒有 session。默默忽略那個選項的話，工作流會以為前情接上了，
    然後在下一輪發現整段消失——而那時候看起來像是「模型忘了」，
    不是「這顆腦根本接不下去」。

    **「沒有工具」那一條在 2026-08-31 被實測推翻**，見 `Test可編輯不再擋`。
    """

    def test_可編輯不再被擋在門口(self, 假端點: str) -> None:
        """**這支原本斷言的是相反的事。**

        舊行為：`_做不到的地方` 看到非唯讀就回「本地模型沒有工具，做不到 edit」。
        實測（同一天，真端點 Qwen3.8-27B-8bit）走完了一整輪回程：
        發 `tool_calls` → 吃 `role: "tool"` 的結果 → 收尾，而且答案正確。

        **那句話講的是 nova 沒給它工具，不是模型不會用。** 工具迴圈補上之後
        這道門檻就是錯的——擋著的話這顆腦永遠做不了實作階段。

        圈禁與權限推導由 `tests/單元/test_本地工具箱.py` 守，
        迴圈本身由 `tests/整合/test_本地工具迴圈.py` 守。
        """
        答 = 本地腦(網址=假端點).詢問("改個檔", 選項=呼叫選項(權限=權限.可編輯))

        assert 答.終局 is 終局.成功, "可編輯又被擋回去了"

    def test_全開也不擋(self, 假端點: str) -> None:
