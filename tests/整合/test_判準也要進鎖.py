...
def test_鎖只圈住跑子程序那一段_模型呼叫照樣跑得動(專屬狀態目錄: Path) -> None:
    ...
    這支盯三件事：`建判準` 不准在建的時候就佔鎖、子程序跑的那一段鎖要在手上、
    跑完就要放掉。中間那段「另一條工作流的模型呼叫」走的是**真的 `跑工作流`**，
    只有那顆腦是假的——模型階段若進了鎖，乙會卡在甲的窗口裡等到 join 逾時。
    """
    旗標 = 專屬狀態目錄 / "放行.旗標"
    甲判準 = 建判準((sys.executable, "-c", _等旗標腳本, str(旗標)), 逾時秒=30.0)
    assert not _鎖被佔著(), "建判準的時候就把鎖佔走了——那把整個階段都圈進去了"
    ...
    try:
        起 = time.monotonic()
        while not _鎖被佔著():
            assert time.monotonic() - 起 < 20.0, "判準跑子程序的時候沒有佔住鎖"
            time.sleep(0.02)
        ...
        assert _鎖被佔著(), "乙講完話的時候甲已經放鎖了——這一輪沒測到重疊"
    finally:
        旗標.write_text("走吧")
        甲.join(timeout=30.0)

    assert 收場們 == [判準終局.綠]
    assert not _鎖被佔著(), "判準跑完沒放鎖——第一次跑完之後所有工作流都卡死"
