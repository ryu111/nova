...
@dataclass(frozen=True, slots=True)
class 替換標記行:
    """錨在**行尾的標記註解**上，把含標記的那一整行換掉。

    錨整行原文的刀，被別票改掉那一行的前半段就當場 `RUN_ERROR`——刀一次都沒真的
    跑過，而表面上看起來有負控。標記註解（`  # 負控錨:<識別>`）合併時不會被順手
    改寫，所以錨得住；標記又落在被變異的那一行上，`_推導破壞行` 算出來的仍是那一行。
    """
    ...
    def 套用(self, 目標檔: Path) -> tuple[str, str]:
        文字 = 目標檔.read_text(encoding="utf-8")
        行們 = 文字.splitlines()
        命中 = [序 for 序, 行 in enumerate(行們) if self.錨點 in 行]
        assert len(命中) == 1, f"標記錨應恰好出現一次，實際 {len(命中)} 次：{self.錨點!r}"
        前 = _雜湊(目標檔.read_bytes())
        行們[命中[0]] = self.變成
        尾 = "\n" if 文字.endswith("\n") else ""
        目標檔.write_text("\n".join(行們) + 尾, encoding="utf-8")
        後 = _雜湊(目標檔.read_bytes())
        assert 前 != 後, "變異前後 SHA256 相同，代表變異沒有生效"
        return 前, 後
...
_這個檔裡的 = (
    ...
    #: R38 commit 那一步搬進 `_有東西才提交`（樹乾淨就不發 commit），
    #: 舊錨點那個「指令 tuple 加逗號」的原文不再存在，錨點跟著改成現在那一行呼叫。
    變異(
        識別="收尾提交不得繞過驗證",
        目標檔=Path("src/nova/載體/命令/收.py"),
        操作=替換一次(
            'return _跑並印收尾指令(根目錄, "git", "commit", "-m", 收尾訊息.全文)',
            'return _跑並印收尾指令(根目錄, "git", "commit", "--no-verify", "-m", 收尾訊息.全文)',
        ),
...
#: 識別 → 這把刀登記在哪個檔（`tests/負控/登記.py` 或 `tests/負控/登記們/<檔名>.py`）。
#:
#: 錨點失效時，訊息只說「錨點應恰好一次」的話，人要在 192 筆登記裡自己 grep 那把刀
#: 住哪個檔。收集當下就知道來源，這裡導出成模組層對照——訊息不必自己再湊一次，
#: 手上只有一筆 `變異`（例如體檢傳進來的那筆）也查得到家。
登記來源: dict[str, str] = {}


def 錨點失效訊息(一筆: 變異, 次數: int) -> str:
    """錨點沒有恰好命中一次時的說法，四件事一次給齊：哪把刀、登記在哪、錨在哪、怎麼重釘。

    執行器與跑前體檢共用這一份：拆成兩份的話兩邊會各說各話。
    """
    來源 = 登記來源.get(一筆.識別, 一筆.來源)  # 負控錨:訊息帶登記檔名
    return (
        f"RUN_ERROR：{一筆.識別}：錨點應恰好一次，實際 {次數}"
        f"（登記在 {來源}，錨在 {一筆.目標檔} 的 {一筆.操作.錨點!r}）；"
        f'去 {來源} 找 識別="{一筆.識別}"，把錨點改成目標檔現在的原文，'
        "或改用標記錨（`替換標記行`）釘在那一行行尾的 `# 負控錨:…` 上"
    )
...
def _擋掉重複識別(每一筆帶來源: Iterable[tuple[變異, str]]) -> tuple[變異, ...]:
    ...
    for 原本那筆, 這筆的來源 in 每一筆帶來源:
        ...
        if 先前的來源 is not None:
            ...
            raise ValueError(訊息)
        來源[一筆.識別] = 這筆的來源
        # **內層那份先寫、外層不准蓋掉**：`_收集` 那次帶的是真檔名；檔尾外層那次
        # 只要退回目錄名，蓋掉的話 192 筆會拿到一模一樣的字串，等於沒說。
        登記來源.setdefault(一筆.識別, 這筆的來源)
        收到.append(一筆)
    return tuple(收到)
