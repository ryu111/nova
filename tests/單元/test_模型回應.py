...
class Test權限被擋不准是確定失敗:
    """負控抓到的洞：把它改成 `確定失敗`，原本 310 支測試全過。

    **沒有會紅的測試背書，那個決定就不是保證，只是一行註解。**

    真正的後果在退出碼：`確定失敗` → `1`（腳本可以重跑），
    `結果未知` → `3`（**腳本不准重跑**）。
    而權限擋掉的是**那一個**工具呼叫，不是整輪——實測過的形狀是
    agy 先 `generate_image` 成功（圖真的產出來了），再 `run_command` 被擋。
    前半做了。當成確定失敗，重跑就會把那個副作用再做一次。
    """

    def test_權限被擋是結果未知(self) -> None:
        assert 終局判定(失敗代碼.權限被擋) is 終局.結果未知

    def test_所以它的退出碼是不准重跑的那個(self) -> None:
        """退出碼才是腳本看得到的東西——保證要釘在使用者摸得到的那一層。"""
        assert _終局的退出碼[終局判定(失敗代碼.權限被擋)] == 未知
        assert 未知 != 閘紅, "這兩個碼要真的不一樣，不然上面那條斷言是空的"
