...
from nova.契約.工作流 import 步驟結果, 種類, 結束, 結束代碼, 階段代碼, 階段定義
from nova.契約.模型回應 import 終局
from nova.迴圈.狀態機 import TDD階段表, 下一步, 查階段


def _結果(終: 終局 = 終局.成功, *, 判準綠: bool | None = None) -> 步驟結果:
    return 步驟結果(階段=階段代碼.測試, 終局=終, 判準綠=判準綠, 證據="")
...
class Test轉移:
    ...
    def test_審查完就結束(self) -> None:
        終 = 下一步(查階段(階段代碼.審查), _結果())
