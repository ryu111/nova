...
class Test每一種收場的token都要算:
    """**`unknown` 的語意是「不知道工作做了沒」——那是講工作，不是講花費。**

    逾時被殺的呼叫 token 已經花掉了，額度那邊照扣。摘要要是漏掉它，
    低估的不是隨機樣本：**跑越久、讀越多、越容易撞逾時**，所以漏掉的
    系統性偏向最貴的那幾次。

    實測過的一輪（`20260830T202750Z-bf4797`）：`success` 那次 375,061→39,925，
    `unknown` 那次 410,294→37,220，總計 862,500。少算 unknown 就變 414,986，
    **低估 52%**。

    收斂端現在是對的（`_記一次呼叫` 沒有 outcome 過濾），但在這支測試之前
    **沒有任何東西守著它**——有人加一個 `if outcome == "success"` 就靜悄悄壞掉，
    而壞掉的樣子是「帳單比實際便宜」，沒有人會來抱怨。
    """

    def test_unknown收場的token算進總計(self) -> None:
        摘 = 收斂(
            [
                *呼叫(1, "agy", 入=375061, 出=39925),
                *呼叫(2, "agy", "unknown", 入=410294, 出=37220),
            ]
        )
        assert 摘.總token == 375061 + 39925 + 410294 + 37220

    def test_失敗收場的token也算(self) -> None:
        """確定失敗一樣花了錢——請求出門了、模型算了、回了一個錯。"""
        摘 = 收斂([*呼叫(1, "agy", 入=100, 出=10), *呼叫(2, "agy", "failure", 入=999, 出=99)])
        assert 摘.總token == 100 + 10 + 999 + 99

    def test_次數與終局分佈要對得上(self) -> None:
        """總計對了但次數錯的話，「平均一次多少」會算錯——那是派工決策的依據。"""
        摘 = 收斂([*呼叫(1, "agy"), *呼叫(2, "agy", "unknown"), *呼叫(3, "agy", "failure")])
        (一家,) = 摘.各家
        assert 一家.次數 == 3

    def test_拿不到token數時當成零_這是已知的極限(self) -> None:
        """**現況記錄，不是理想行為。**

        CLI 被 kill 前沒來得及回報 token 時，收斂端把那一筆當 0
        （`_記一次呼叫` 的 `isinstance(值, int)` 為假就不加）。
        所以摘要分不出「花了 0」與「不知道花多少」——會低估，不會高估。

        要分開得在契約加一個「未知」狀態，那是還沒有呼叫端要求的東西。
        先把現況釘住：改動這個行為會讓這支紅，屆時再決定要不要往那個方向走。
        """
