...
class Test逾時的回應:
    ...
    def test_codex開場白不是停滯位置(self) -> None:
        """開場白是證據，不是狀態；只有它時還要明說沒有實質回應。"""
        答 = 逾時的回應(
            _沒文字的解析,
            "",
        ...
        assert 答.文字.splitlines() == [
            "結果未知：這次呼叫跑了 1800.0 秒，上限 1800 秒；不准重跑，去看工作區。",
            "子程序最後印出（不代表停在這裡）：Reading additional input from stdin...",
            "子程序沒有實質回應；這次只收到開場白。",
        ]
