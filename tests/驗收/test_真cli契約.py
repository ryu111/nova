...
def _乾淨工作區(tmp: 路徑) -> 路徑:
    """給模型一個**專屬**的工作目錄，不要直接用 `tmp_path`。

    `tmp_path` 本身被 conftest 的 `帳本不准寫到家目錄` 拿去放 `XDG_STATE_HOME`
    了。斷言「這個目錄整個是空的」時，那個 `state/` 會被當成「模型動了檔案系統」
    ——**斷言範圍比要守的東西大**，跟負控紀錄裡那三次假紅是同一種。
    """
    區 = tmp / "工作區"
    區.mkdir()
    return 區
...
def test_claude隔離設定之後拿不到自動注入的指引(tmp_path: 路徑) -> None:
    ...
    **「自動注入」與「主動讀取」是兩件事，這支只管前者。** 原本的版本在 repo 根目錄
    ...
    答 = 建立("claude", 執行檔=None).詢問(
        ...
        選項=呼叫選項(隔離設定=True, 工作目錄=_乾淨工作區(tmp_path), 逾時秒=180.0),
    )
...
def test_唯讀真的寫不出檔案(家: str, tmp_path: 路徑) -> None:
    ...
    工作區 = _乾淨工作區(tmp_path)
    答 = _叫它寫檔(家, 工作區, 權限.唯讀)
    落地 = sorted(p.name for p in 工作區.iterdir())
