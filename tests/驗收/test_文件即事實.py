...
#: 刪除線＝「這是當時的名字」。只赦免它自己包住的那一段，不跨行。
_刪除線 = re.compile(r"~~.*?~~")


def _宣稱的測試名(原文: str) -> list[str]:
    """抓出文件宣稱「這支現在還在」的測試名。

    被刪除線包住的測試名是帳本裡的歷史記述，不是宣稱，所以先把那幾段拿掉。
    """
    return _測試名.findall(_刪除線.sub("", 原文))
...
def _負控紀錄第一層檔們() -> list[Path]:
    """`docs/負控紀錄/` 底下的主題檔，跟設計、決策一樣**只掃一層**。

    `test_負控紀錄拆檔.py` 故意**不叫這支**、自己數一次目錄：
    oracle 跟被驗的收集器共用同一份 glob 的話，這支漂成寫死清單也照樣綠。
    """
    return sorted(檔 for 檔 in 負控紀錄目錄.glob("*.md") if 檔.is_file())
...
def _要檢查的文件() -> list[Path]:
    ...
    負控紀錄是**整個目錄**掃進來的，不是寫死一份清單：新線只丟一個
    `docs/負控紀錄/<主題>.md` 就自動受檢，不必再 append 同一張表的尾巴。
    寫死清單的話今天綠、下一條線加檔就假綠——`test_每個主題檔都被文件檢查收走`
    守著這一點。
    """
    return [
        專案根目錄 / "CLAUDE.md",
...
def test_文件提到的測試都真的存在(測試名們: set[str]) -> None:
    ...
    缺的 = [
        f"{文件.name} 提到 {名}"
        for 文件 in _要檢查的文件()
        for 名 in _宣稱的測試名(文件.read_text(encoding="utf-8"))
        if 名 not in 測試名們
    ]
...
def test_標記過的舊測試名不算宣稱() -> None:
    """帳本記的是**當時**做過什麼；刪除線包住的名字是歷史記述，不是宣稱。

    `docs/負控紀錄/` 是帳本：「那一格當時叫 X」是既成事實，
    而 X 後來被改名是**正常的**，不該讓 `test_文件提到的測試都真的存在` 紅。
    在此之前唯一的繞法是「故意不加反引號」，
    還得附一段註解求下一個人別好心把它「修好」——
    **只以文字存在的規範等於不存在**，所以要有語法、要有閘。

    記號沿用本專案文件裡既有的刪除線（`docs/設計/03-角色與工作流.md`
    的未解問題清單用它表示「這條不再成立」），不必再教一次。
    """
    原文 = "那一格當時叫 ~~`test_審查用是必要參數`~~，`#141` 把規則反轉之後改掉了。"
    assert _宣稱的測試名(原文) == []


def test_沒標記的測試名還是照抓() -> None:
    """刪除線只赦免它自己包住的那一個，閘不准因此變寬。

    這支是上一支的配對負控：把 `_宣稱的測試名` 寫成「這一行有刪除線就整行跳過」
    或「乾脆別抓了」，上一支都會綠——而那會讓真正的漂移
    （文件宣稱某支測試存在、它其實已經不在）從此沒有人擋。
    """
    原文 = (
        "當時叫 ~~`test_審查用是必要參數`~~，現在由 `test_同一家不再擋在門口` 守，"
        "另外 `test_這支從來沒有人寫過` 也在守。"
    )
    assert _宣稱的測試名(原文) == [
        "test_同一家不再擋在門口",
        "test_這支從來沒有人寫過",
